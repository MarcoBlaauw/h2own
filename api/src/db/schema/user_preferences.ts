...
export const userPreferences = pgTable('user_preferences', {
  ...
  billingCustomerId: varchar('billing_customer_id', { length: 128 }),
  billingSubscriptionId: varchar('billing_subscription_id', { length: 128 }),
  billingProvider: varchar('billing_provider', { length: 40 }),
  billingPaymentStatus: varchar('billing_payment_status', { length: 24 }).notNull().default('unpaid'),
  ...
  notificationPhoneNumber: varchar('notification_phone_number', { length: 32 }),
  notificationSmsVerified: boolean('notification_sms_verified').notNull().default(false),
  notificationPushDeviceRegistered: boolean('notification_push_device_registered').notNull().default(false),
