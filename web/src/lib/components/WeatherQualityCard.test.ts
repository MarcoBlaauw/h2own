...
describe('WeatherQualityCard', () => {
  ...
  it('shows key labels for empty state', () => {
    const { getAllByText, getByText } = render(WeatherQualityCard, {
      props: { dailyWeather: [] },
    });

    expect(getByText('No data')).toBeTruthy();
    expect(getAllByText('Connect a location to see forecasts.')).toHaveLength(2);
  ...
  it('shows key labels for unavailable state', () => {
    ...
    expect(getAllByText('Location is missing coordinates')).toHaveLength(2);
  });
